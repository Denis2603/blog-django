...
class LogoutTest(TestCase):
    ...
    def test_view_url_exists_at_desired_location(self):
        self.client.force_login(self.test_user)
        resp = self.client.get('/users/logout/', follow=True)
        self.assertEqual(resp.status_code, 200)
...
class UserRegistrationViewTest(TestCase):
    ...
    def test_view_url_exists_at_desired_location(self):
        resp = self.client.get('/users/reg/')
        self.assertEqual(resp.status_code, 200)
...
class ProfileViewTest(TestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.text_about_old = 'Старая инфо о пользователе'
        cls.text_about_new = 'Измененная инфо о пользователе'
        ...
        cls.test_user = User.objects.create_user(username='nik', password='111', first_name='Иван', last_name='Петров')
        cls.test_profile = Profile.objects.create(user=cls.test_user, about=cls.text_about_old, ava=cls.test_avatar)
    ...
    def test_change_about(self):
        ...
        self.assertEqual(current_user.profile.about, self.text_about_old)
        self.client.post(reverse('profile'), data={'about': self.text_about_new})
        current_user.refresh_from_db()
        self.assertEqual(current_user.profile.about, self.text_about_new)

    def test_change_avatar(self):
        current_user = User.objects.get(id=1)
        # проверка изначально содержимое файла 000, как задано в setUpTestData(cls):
        ...
        self.assertEqual(current_user.profile.ava.url, '/media/users/foto/nik.jpg')
